...
class MultiAgentSearchAgent(CaptureAgent):
    ...
    #    distanceToClosestFood = min(map(lambda x: getDistanceInMaze(newPos, x), oldFood.asList()))
        ...
        return foodScore + ghostScore +captureScore
    ...
    def isLost(self,gameState,enemies):
        ...
        if len(defenders) > 0:
            ...
            return (self.getFoodYouAreDefending(gameState)<=2 or min(distance_to_defenders)<1)
        else:
...
class MinimaxAgent(MultiAgentSearchAgent):
  ...
  def gameOver(self, gameState, d):
    ...
    return isLose(gameState,enemies) or isWin(gameState) or d == 0
